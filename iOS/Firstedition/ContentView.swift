...
struct EntryRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            Text("Edition: \(entry.edition)  ·  Condition: \(entry.condition)")
                .font(.caption)
                .foregroundColor(Theme.textSecondary)
            Text("Est. Value: \(entry.value)")
                .font(.caption)
...
struct EntryFormView: View {
    ...
    init(mode: FormMode) {
        ...
        if case .edit(let entry) = mode {
            ...
            _f1 = State(initialValue: entry.edition)
            _f2 = State(initialValue: entry.condition)
            _f3 = State(initialValue: entry.value)
        }
    }
        ...
        NavigationStack {
            Form {
                Section("Details") {
                    ...
                    TextField("Edition", text: $f1)
                        .accessibilityIdentifier("f1Field")
                    TextField("Condition", text: $f2)
                        .accessibilityIdentifier("f2Field")
                    TextField("Est. Value", text: $f3)
